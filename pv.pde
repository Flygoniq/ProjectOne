...
class pt 
  ...
  pt restrictedMoveWithMouse() {
    if ((x + mouseX - pmouseX) > (y + mouseY - pmouseY)) {
      x += mouseX-pmouseX; y += mouseY-pmouseY;
    }
    return this;
  }
  pt invert() {
    return new pt(center.x + (center.x - x),center.y + (center.y - y));
  }
...
void showId(pt P, String S) {fill(white); show(P,10); fill(black); label(P,S);}                       // sows disk with S written inside
